  ...
  template: `
    <span class="flex items-center gap-2" [class]="disabled() ? 'cursor-not-allowed' : 'cursor-pointer'" (click)="onCheckboxChange()">
      <main class="flex relative">
        ...
        <span
          class="checkbox-check absolute flex items-center justify-center inset-0 text-primary-foreground opacity-0 peer-checked:opacity-100 pointer-events-none text-[0.75rem] font-bold leading-none"
          aria-hidden="true"
        >✓</span>
      </main>
